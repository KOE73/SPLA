...
**Фаза 2.2 (следующее, протокольная, самая рискованная часть):** сейчас `ClientConnection`/
`SplaServiceHost` держат ОДИН `_runtime`/`_chats` на всё соединение — `SettingsOps`,
`ConnectionDiagOps`, workspace-fs-browse, schema-get все читают `_runtime` напрямую. Чтобы клиент
мог переключаться между проектами в одном сокете, нужно: (1) `ProjectOps` в протоколе
(project.list/recent/open/create ↔ результаты), (2) решить как chatId резолвится в ProjectId на
чат-scoped сообщениях (chat.send и т.п.) — вариант А: клиент шлёт ProjectId явно при chat.new/
chat.open, сервер запоминает связку chatId→ProjectId на дальнейшие сообщения этого чата;
вариант Б: ProjectId обязателен в КАЖДОМ чат-конверте (проще для сервера, чуть больше протокола).
Это ломает предположение «один _runtime на соединение» почти во всех `*Ops`-классах — сознательно
не начато в эту сессию: протокол нельзя провалидировать вживую (нет UI, который бы переключал
проекты), а трогать десяток файлов без возможности проверить в браузере — риск оставить
недоделанную, невалидированную проводку под видом «готово».
