...
**Фаза 2.1 (следующее):** `ProjectOps` в протоколе сервиса (List/Recent — read-only первым
шагом; Open/Create требуют multi-runtime: сейчас AgentRuntime жёстко один-проект-на-процесс).
UI: дерево проекты→чаты в web-клиенте поверх ProjectOps. Перевод UI.Avalonia/CLI с
`recent_projects.txt` на провайдер.
