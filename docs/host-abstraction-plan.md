...
**Фаза 1.2 (следующее):** плагины получают бакет при init через `settings.Project`
(OneC sqlite → bucket, Browser profile → bucket) СИНХРОННО с расширением
`AvaloniaPluginContext` (сейчас он несёт только `WorkspacePath`, и панели клеят путь к той же
sqlite руками — мигрировать по одному нельзя, разведёт пути). `IconGenerator` — не трогаем
(читает пользовательский `.spla/icon.png`, косметика нативного UI).
