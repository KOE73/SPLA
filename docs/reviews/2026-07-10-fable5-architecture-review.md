...
passthrough: [ChatRuntime.cs:112](../../src/service/SPLA.Service/ChatRuntime.cs) создаёт
...
- **Потеря tool-linkage при персисте**: `ChatSessionMessage` хранит role/content/reasoning/images,
  но не `ToolCalls`/`ToolCallId` ([ChatRuntime.Save](../../src/service/SPLA.Service/ChatRuntime.cs)).
...
1. [agents/security.md:4](../../agents/security.md) ссылается на
   `src/service/SPLA.MCP.Core/Permissions/PermissionManager.cs` — фактический путь
