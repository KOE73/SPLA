...
**Не влияет на живого наблюдателя (`ADR_20260910-2`, волна 1+).** Окно/консоль/`chat run`, открытые
посреди хода, больше не читают файл — снимок для `chat.opened` берётся из памяти сеанса
(`ChatFeed.SnapshotUnderGate`, см. `agents/chat-feed.md`), так что живой ход виден вживую независимо
от того, когда именно пишется файл. Само это issue не закрыто: падение процесса или отмена
по-прежнему теряют ход целиком — поток живёт только в памяти процесса, а не на диске.
