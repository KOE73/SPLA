...
**Незакрытый хвост:** до этой правки клик по архивному чату в `ChatList.vue` (`onChatClick`, общий
для обеих секций списка) *работал* — потому что `chat.open` попутно снимал архивацию де-факто. Теперь
он получает ошибку и не показывает содержимое вообще. Просмотр архивного чата без снятия архивации —
отдельная работа (нужен путь чтения без создания `ChatRuntime`, см. обсуждение варианта «Read vs
Open» в переписке 2026-09-03), сюда сознательно не включена.
