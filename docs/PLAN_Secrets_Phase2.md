...
- **`ssh_run`** — одна **read-only** команда на хосте, stdout/stderr/exit. Гвард `ReadOnlyGuard`
  проверяет команду **до** коннекта.
...
редакция секретов в логах агента, TLS для не-loopback. Следующий крупный шаг по SSH — «живая консоль»
(pty-сессия, видимая и прерываемая человеком): см. [DESIGN_Secrets_and_SSH.md](DESIGN_Secrets_and_SSH.md).
