...
- **[Russian Terminology](agents/glossary.md)**: STOP — read this before writing Russian prose in an ADR, a plan, a diagram text catalogue, a readme or a UI string, and before translating an English term of art. The repository codes in English and documents in Russian; that seam is where an idiom gets rendered by dictionary and starts naming a mechanism from another industry (`ChatPump` → «насос ходов», caught after it had spread to four files). Translate the role, not the metaphor; identifiers stay English.
