...
public sealed class BrowserConsoleTool : IMcpTool
{
    public string Name => "browser_console";
    ...
    public ToolDefinition GetDefinition() => new()
    {
        ...
        {
            Name = Name,
            Description = "Recent browser console messages (log/warn/error) for a tab. Useful to diagnose why a page misbehaves.",
    ...
    public Task<string> ExecuteAsync(string argumentsJson, CancellationToken cancellationToken = default)
    {
        ...
        var entries = mgr.DiagnosticsFor(resolvedTabId!).Console();
        return Task.FromResult(entries.Count == 0
            ? $"No console messages recorded for tab {resolvedTabId}."
            : string.Join("\n", entries));
    }
}
