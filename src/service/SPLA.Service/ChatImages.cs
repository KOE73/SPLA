...
/// <summary>
/// Sidecar storage for chat image attachments. Images live in the project's
/// <c>chat-images</c> bucket, one subfolder per chat; the chat YAML keeps only filenames, so the
...
public static class ChatImages
{
    ...
    public static string Dir(IProject project, string chatId)
        => Path.Combine(
            project.GetBucket("chat-images").MapToHostDirectory()
                ?? throw new InvalidOperationException("Chat images need a disk-backed project backend."),
            chatId);
    ...
    public static string? WriteDataUrl(IProject project, string chatId, string dataUrl)
    {
    ...
    public static string? Resolve(IProject project, string chatId, string fileName)
    {
        ...
        var full = Path.Combine(Dir(project, chatId), fileName);
