...
/// <param name="Kind">What this participant is; one of <see cref="ParticipantKind"/>. Defaults to
/// <see cref="ParticipantKind.Agent"/> because the file backend can only ever see agents — a lock
...
public sealed record InstanceRecord(
    ...
    InstanceState State,
    int? Clients,
    string Kind = ParticipantKind.Agent)
{
