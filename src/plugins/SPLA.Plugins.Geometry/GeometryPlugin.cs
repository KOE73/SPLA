...
public sealed partial class GeometryPlugin : ISplaPlugin
{
    ...
    {
        Settings = settings;
        return [new GeometryOpenTool(settings), new GeometryBoxTool(settings), new GeometryPointTool(settings), new GeometryViewTool(settings), new GeometryAcceptTool(settings), new GeometryResultTool(settings)];
    }
}
