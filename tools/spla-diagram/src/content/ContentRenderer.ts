...
function drawDirective(
  ...
  switch (cell.name.toLowerCase()) {
    case "name":
      return line(data.name, x, y, width, style.title, "spla-node-label");

    case "description":
      return line(data.description ?? "", x, y, width, style.subtitle, "spla-node-subtitle");
...
function members(
  ...
  if (cell.args.named["collapsed"] === true) {
    return line(`▸ ${selected.length} членов`, x, y, width, look, "spla-node-members-collapsed");
  }
...
function asset(
  ...
  const holder = svg("g", { class: "spla-node-asset", transform: `translate(${x} ${y})` });
