...
- `SPLA.Registry.Contracts`: What a registry answers with — `InstanceRecord`, `IInstanceRegistry`, `IInstanceProbe`. Deliberately free of any hosting dependency: the desktop shell references it to draw a tray, and must never acquire ASP.NET to do so. Its one project reference is `SPLA.Domain`, for `InstanceInfo`/`InstanceState` — those describe a project's own lock file, and a second definition would be a second truth.
